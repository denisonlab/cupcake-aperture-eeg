...
%set up audio handle. make sure anything open is closed and open new ones
...
if p.triggerEEG, ppdev_mex('Write', 1, p.triggers.fixation); end
timeFix = Screen('Flip', window); % first image will be presented after an ITI with respect to this time
timing.startTime = timeFix;
if p.triggerEEG, ppdev_mex('Write', 1, 0); triggerTimes = [triggerTimes; p.triggers.fixation GetSecs]; end
if p.triggersOn
...
for iTrial = 1:nTrials
    ...
    if p.triggerEEG, ppdev_mex('Write', 1, p.triggers.image); end
    timeIm = Screen('Flip', window, timeFix + iti - slack);
    if p.triggerEEG, ppdev_mex('Write', 1, 0); triggerTimes = [triggerTimes; p.triggers.image GetSecs]; end
    if p.triggersOn
